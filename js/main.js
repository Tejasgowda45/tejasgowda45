...
let index = 0; // To track the current title in the list
let charIndex = 0; // To track the current character in the title
...
const delayBetweenTitles = 1500; // Delay before starting to delete
const delayBeforeTyping = 500; // Delay before starting to type

function typeEffect() {
	if (isDeleting) {
		if (charIndex > 0) {
			dynamicTextElement.textContent = titles[index].substring(0, charIndex - 1);
			charIndex--;
		...
		if (charIndex < titles[index].length) {
			dynamicTextElement.textContent += titles[index][charIndex];
			charIndex++;
